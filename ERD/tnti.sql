...
SELECT COUNT(*) FROM  OFFICE_HOUR WHERE EMP_UID = 3 AND TO_CHAR(W_START,'yyyy-mm-dd')= TO_CHAR(SYSDATE, 'yyyy-mm-dd') ;
SELECT COUNT(*) FROM  OFFICE_HOUR WHERE EMP_UID = 3 AND TO_CHAR(W_END,'yyyy-mm-dd')= TO_CHAR(SYSDATE, 'yyyy-mm-dd') ;
SELECT W_UID, W_START,W_END, EMP_UID FROM  OFFICE_HOUR WHERE EMP_UID = 4;


SELECT  TRUNC(SYSDATE, 'dd' ) FROM dual;

		update OFFICE_HOUR set w_end = SYSDATE where emp_uid = 3 AND TO_CHAR(SYSDATE, 'yyyymmdd') = (SELECT to_char(W_START, 'yyyymmdd') FROM OFFICE_HOUR WHERE EMP_UID = 3)