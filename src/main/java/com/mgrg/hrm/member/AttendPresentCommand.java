...
public class AttendPresentCommand implements Command {
	...
	public void execute(Model model) {
		...
		if(dao.loginCount(uid) > 0) {
			message.append("오늘(" + inDate + ") 이미 출근하셨습니다.");
		} else {
			result = dao.present(uid);
			message.append("현재(" + inDate + ") 시간으로 출근처리 되었습니다.");
			status = "OK";
		} // end if-else (출석여부)
